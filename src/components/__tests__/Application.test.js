...
import { render, cleanup, waitForElement, prettyDOM, getByText, getAllByTestId, getByAltText, getByPlaceholderText, queryByText, queryByAltText, getByTestId } from "@testing-library/react";
...
describe("Appointments", () => {
  ...
  it("shows the save error when failing to save an appointment", async () => {
    ...
    // 3. Click the "Add" button to add an appointment.
    fireEvent.click(getByAltText(appointment, "Add"));

    // 4. Enter students name - value = Lydia Miller-Jones
    fireEvent.change(getByPlaceholderText(appointment, /Enter student name/i), {
      target: { value: "Lydia Miller-Jones" }
    });

    // 5. Select an intterviewer
    fireEvent.click(getByAltText(appointment, "Sylvia Palmer"));

    // 6. Click the "Save" button on the edited appointment
    fireEvent.click(getByText(appointment, 'Save'));

    // 7. Wait until the text "Lydia Miller-Jones" is displayed.
    await waitForElement(() => getByText(appointment, "Cannot save appointment"));
    expect(getByText(appointment, "Cannot save appointment")).toBeInTheDocument();

    // 5. Click the "Close" button on the appointment
